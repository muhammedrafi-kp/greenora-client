...
import { getCollectorData, getPaymentData } from '../../services/userService';
...
const CollectionDetails: React.FC = () => {
    ...
    const handleConfirmCancel = () => {
        ...
        // TODO: Implement API call to cancel collection with reason
        console.log('Cancelling collection with reason:', finalReason);
        setShowCancelModal(false);
    };

    const { icon, color } = getStatusIconAndColor(collectionDetails.status);
    const { icon: paymentIcon, color: paymentColor } = getPaymentStatusIconAndColor(collectionDetails.paymentStatus);
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className={`flex items-center gap-2 px-3 py-1 rounded-full ${paymentColor}`}>
                                    {/* {paymentIcon} */}
                                    <span className="text-sm font-medium">{collectionDetails.paymentStatus.charAt(0).toUpperCase() + collectionDetails.paymentStatus.slice(1)}</span>
                                </div>
                            </div>
                            <div className="flex items-center justify-between">
                    ...
                    {collector && (
                        <div className="bg-white rounded-lg shadow-md p-6 border">
                            ...
                            <div className="mt-4 space-y-4">
                                ...
                                <button onClick={() => navigate(`/collector-tracking/${collector._id}`)} className="w-full py-2 bg-gray-800 text-white rounded-lg hover:bg-gray-900 transition-colors flex items-center justify-center gap-2">
